...
async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      ...
      if (!objectId) {
        return NextResponse.json({ success: false, error: 'Ranking ID is invalid' }, { status: 400 });
      }
...
async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Failed to create ranking' }, { status: 500 });
  }
}
