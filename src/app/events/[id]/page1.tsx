...
export default async function Page({
  ...
  const canonicalSlug = typeof event?.slug === "string" ? event.slug : "";
  if (
    canonicalSlug &&
    canonicalSlug !== "all-events" &&
    ObjectId.isValid(id) &&
    canonicalSlug !== id
  ) {
    permanentRedirect(`/events/${canonicalSlug}`);
  }
              ...
              {relatedAward && (
                <div className="pt-6 border-t border-border-light mb-8">
                  <div className="bg-gradient-to-br from-gold/10 to-gold/5 rounded-2xl p-6 border border-gold/20 shadow-sh-xs">
                    ...
                    <Link
                      href={`/nomination-form/${awardData?.slug}?id=${relatedAwardIdStr}`}
                      className="block"
                    >
